...
//
// Discriminator (canonical filter): self-author = generation_key IS NULL AND
//   metadata->>'regime' IN ('C','C-prime'); pipeline = generation_key IS NOT NULL.
// Excludes legacy-sqlite (migrated 12-14/mai), manual saves (save_observation
// sem checkpoint_key), and pre-tag self-author — all generation_key NULL but
// NOT part of the instrumented regime experiment. See THREE-WAY-REGIMES.md.
import pg from "pg";
...
const daily = await q(
  `select to_char(created_at at time zone 'UTC','YYYY-MM-DD') date,
          count(*) filter (where ${SA}) self_authored,
...
const base = await q(
  `select case when generation_key is not null then 'pipeline'
               when ${SA} then 'self_author' else 'other' end cls,
          count(*) n, round(avg(length(content))) avg_chars,
...
const metaSA = (await q(`select count(*) n from observations where project_id=$1 and created_at>=$2 and ${SA} and metadata->>'title' ~* $3`, [PROJ, SINCE, re]))[0].n;
...
const pp = base.find(r => r.cls === 'pipeline'), sa = base.find(r => r.cls === 'self_author');
...
const kinds = await q(
  `select kind, count(*) filter (where ${SA}) self_author,
...
const cum = (await q(`select count(*) n from observations where project_id=$1 and ${SA}`, [PROJ]))[0].n;
const tot = (await q(`select count(*) n from observations where project_id=$1`, [PROJ]))[0].n;
console.log(`window since ${SINCE}: pipeline=${pp.n} self_author=${sa.n} (canonical: regime IN C,C-prime)`);
console.log(`all-time self_author(instrumented)=${cum}/${tot}; daily rows=${daily.length}`);
await c.end();
