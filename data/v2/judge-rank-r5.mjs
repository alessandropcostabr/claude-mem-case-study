...
const B = byReg('C-Stop'), C = byReg('C-prime-inc'), CP = byReg('C-prime-arc');
...
const rankSum = { 'C-Stop': 0, 'C-prime-inc': 0, 'C-prime-arc': 0 }, lastCount = { 'C-Stop': 0, 'C-prime-inc': 0, 'C-prime-arc': 0 }, firstCount = { 'C-Stop': 0, 'C-prime-inc': 0, 'C-prime-arc': 0 };
...
for (let i = 0; i < N; i++) {
  ...
  const regOf = src => ['C-Stop','C-prime-inc','C-prime-arc'][src];
...
for (const reg of ['C-prime-arc','C-Stop','C-prime-inc'])
  done && console.log(`  ${reg.padEnd(8)} mean_rank=${(rankSum[reg]/done).toFixed(2)}  #1st=${firstCount[reg]}  #last=${lastCount[reg]}`);
done && console.log(`  arc-1st = ${firstCount['C-prime-arc']}/${done} = ${(100*firstCount['C-prime-arc']/done).toFixed(0)}%`);
